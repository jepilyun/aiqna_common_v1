...
export type TYouTubeTranscriptSnippet = {
  ...
  runs?: TYouTubeTranscriptTextUnit[]; // 텍스트 실행 단위들의 배열 (스타일링된 텍스트 등)
};
...
export type TYouTubeTranscriptSegmentRenderer = {
  snippet?: TYouTubeTranscriptSnippet; // 텍스트 스니펫
  text?: string; // 직접적인 텍스트 (대안)
  runs?: TYouTubeTranscriptTextUnit[]; // 텍스트 실행 단위들 (대안)
 ...
 * };
 */
export type TYouTubeTranscriptAnySegment =
 ...
 */
export type TYouTubeTranscriptStandardSegment = {
 ...
 * }
 */
export type TYouTubeTranscriptStandardFormat = {
  videoId: string;                           // YouTube 비디오 ID
  language: string;                          // 자막 언어 코드 (예: 'ko', 'en')
  segments: TYouTubeTranscriptStandardSegment[];    // 시간별 자막 세그먼트 배열
}
